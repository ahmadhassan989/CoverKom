...
const form = useForm({
  user_message: props.order.user_message,
});
...
<template>
  <div class="p-2 rounded m-1 bg-white  flex space-x-2">
    <svg
    ...
    <div class="flex-1">
      <div class="flex justify-between items-center">
        ...
        <Dropdown
          v-if="order.user.id === $page.props.auth.user.id"
          align="left"
        >
          <template #trigger>
            <button>
      ...
      <p v-else class="mt-4 text-lg text-gray-900">{{ order.user_message }}</p>
    </div>
  </div>
</template>
